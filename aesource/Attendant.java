...
class Attendant {
    ...
    private CardReader cardReader;//Added by Christopher Chianelli; card reader (so we can move up or down the chain)
    ...
    */
    ...
    public boolean processActionCard(Card c) {
        ...
        //Added by Christopher Chianelli; cards that allow the attendant to
        //modify the tape and remember a small stack; allows recursive function calls
        else if (card.toLowerCase().startsWith(" increment variable cards by", 1))
        {
        	int amount = Integer.parseInt(card.substring(30));
        	for (Card ca : (Vector<Card>) cardChain)
        	{
        		if (ca.text.startsWith("N") ||
        				ca.text.startsWith("L") ||
        				ca.text.startsWith("S") ||
        				ca.text.startsWith("Z"))
        		{
        			if (ca.text.charAt(1) != '9')
        			{
        				ca.text = ca.text.charAt(0) + String.format("%03d", Integer.parseInt(ca.text.substring(1,4)) + amount) + ca.text.substring(4);
        			}
        		}
        	}
        ...
        else if (card.toLowerCase().startsWith(" decrement variable cards by", 1))
        {
        	int amount = Integer.parseInt(card.substring(30));
        	for (Card ca : (Vector<Card>) cardChain)
        	{
        ...
        else if (card.toLowerCase().startsWith(" returns to location on the top of the stack", 1))
        {
        	int topOfStack = callStack.remove(callStack.size() - 1);
        	int offset = topOfStack - c.index;
        	if (offset >= 0)
        		cardReader.advance(offset);
        	else
        ...
        //End of update by Christopher Chianelli    	
        else {
            ok = false;
        }
        if (!ok) {
